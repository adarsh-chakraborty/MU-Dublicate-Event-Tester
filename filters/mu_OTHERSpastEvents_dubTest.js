...
async function getPastEvents() {
    ...
    while (currentPage <= totalPages) {
      ...
      const response = await fetch(`http://localhost:32000/api/getPastEventsMastersunion?page=${currentPage}&eType=OTHERS`);
  ...
  getPastEvents()
    .then(allEvents => {
        ...
        fs.writeFileSync("mu_OTHERSpastEvents_dubTest.json", JSON.stringify(allEvents));
