...
async function getPastEvents() {
    ...
    while (currentPage <= totalPages) {
      ...
      const response = await fetch(`http://localhost:32000/api/getPastEventsMastersunion?page=${currentPage}&eType=Webinar`);
      ...
      const { pastEvents, totalPages: total, currentPage: page } = responseData.Data;
  ...
  getPastEvents()
    .then(allEvents => {
        ...
        fs.writeFileSync("mu_WebinarsPastEvents_dubTest.json", JSON.stringify(allEvents));
